      ...
      <DialogContent className="max-w-lg">
        ...
        {!isLoading && !isError && hubs?.length === 0 && (
          <p className="text-center text-sm text-muted-foreground py-10">
            Hiện chưa có hub nào hoạt động.
          </p>
        )}
            ...
            {hubs.map(hub => {
              ...
              return (
                <li key={hub.id}>
                  ...
                  >
                    <MapPin
                      className={`mt-0.5 h-4 w-4 shrink-0 ${isSelected ? 'text-primary' : 'text-muted-foreground group-hover:text-emerald-500'}`}
                    />
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate mt-0.5">{hub.address}</p>
                      {hub.phoneNumber && (
                        <p className="text-xs text-muted-foreground mt-0.5">{hub.phoneNumber}</p>
                      )}
                    </div>
                    {isSelected && (
