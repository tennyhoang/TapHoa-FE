...
const SIDEBAR_CATS = [
  { icon: '🦐', label: 'Tôm các loại' },
  { icon: '🐟', label: 'Cá hồi Na Uy' },
  { icon: '🧊', label: 'Hải sản đông lạnh' },
  { icon: '🌊', label: '100% tươi sống' },
  { icon: '🥫', label: 'Nước sốt gia vị' },
  { icon: '🦞', label: 'Tôm hùm' },
  { icon: '🐙', label: 'Bạch tuộc' },
  { icon: '🦪', label: 'Nghêu sò ốc hến' },
];
...
const SLIDES = [
  {
    ...
    sub: 'Trực tiếp từ ngư dân — giao trong ngày',
    cta: 'Mua ngay',
    href: '/',
    emoji: '🐟',
  },
  {
    ...
    sub: 'Nhập khẩu trực tiếp — đảm bảo chất lượng',
    cta: 'Khám phá',
  ...
  },
  {
    ...
    sub: 'Tiết kiệm hơn khi mua combo — giao miễn phí',
    cta: 'Xem combo',
    href: '/',
    emoji: '🦀',
  },
          ...
          {SIDEBAR_CATS.map(cat => (
            <li key={cat.label}>
              ...
              >
                ...
                <ChevronRight className="h-3.5 w-3.5 opacity-0 group-hover:opacity-100 text-blue-500 transition-opacity shrink-0" />
              </Link>
            </li>
          ))}
        </ul>
          ...
          {SLIDES.map((s, i) => (
            ...
            >
              <div className="h-full flex items-center px-10 md:px-14 gap-6">
                ...
                <span className="hidden md:block text-[100px] opacity-90 shrink-0">{s.emoji}</span>
              </div>
            </div>
          ))}
