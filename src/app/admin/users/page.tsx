...
function EditUserDialog({ user, open, onClose }: { user: AdminUser | null; open: boolean; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => userService.update(user!.id, { fullName, phoneNumber: phone, role, isActive }),
    onSuccess: () => {
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={phone} onChange={e => setPhone(e.target.value)} className="h-9 text-sm" placeholder="0912..." />
          </div>
          <div className="space-y-1.5">
...
export default function AdminUsersPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn:  () => userService.getAll({ page, pageSize: PAGE_SIZE, search: search || undefined, role: roleFilter || undefined }),
        ...
        {totalPages > 1 && (
          <div className="flex justify-between items-center px-4 py-3 border-t border-border/40 bg-muted/20">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" disabled={page === 1} onClick={() => setPage(p => p - 1)} className="h-8 text-xs">Trước</Button>
              <Button variant="outline" size="sm" disabled={page === totalPages} onClick={() => setPage(p => p + 1)} className="h-8 text-xs">Sau</Button>
            </div>
          </div>
        )}
      </div>
