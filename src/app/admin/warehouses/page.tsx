...
import { Warehouse as WarehouseIcon, Plus, Pencil, Trash2, ToggleLeft, ToggleRight } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { warehouseService, type Warehouse, type WarehousePayload } from '@/services/warehouse.service';

const EMPTY: WarehousePayload = { name: '', address: '', ward: '', district: '', province: '', phoneNumber: '' };
...
function WarehouseForm({
  ...
  const valid = form.name.trim() && form.address.trim() && form.ward.trim() && form.district.trim() && form.province.trim();
    ...
    <div className="space-y-3 p-4 bg-muted/30 rounded-xl border border-border">
      <div className="grid sm:grid-cols-2 gap-3">
        <div className="space-y-1.5 sm:col-span-2">
          ...
          <Input value={form.name} onChange={set('name')} placeholder="VD: Kho Trung Tâm TP.HCM" className="h-9" />
        </div>
        <div className="space-y-1.5 sm:col-span-2">
          <label className="text-xs font-medium text-muted-foreground">Địa chỉ (số nhà, tên đường) *</label>
          <Input value={form.address} onChange={set('address')} placeholder="VD: 227 Nguyễn Văn Cừ" className="h-9" />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Phường/Xã *</label>
          <Input value={form.ward} onChange={set('ward')} placeholder="VD: Phường 4" className="h-9" />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Quận/Huyện *</label>
          <Input value={form.district} onChange={set('district')} placeholder="VD: Quận 5" className="h-9" />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Tỉnh/Thành phố *</label>
          <Input value={form.province} onChange={set('province')} placeholder="VD: TP.HCM" className="h-9" />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Số điện thoại</label>
          <Input value={form.phoneNumber ?? ''} onChange={set('phoneNumber')} placeholder="VD: 0901234567" className="h-9" />
        </div>
      </div>
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Danh sách kho hàng, tài xế chọn kho xuất phát khi giao hàng</p>
        </div>
        {!showForm && (
          <Button onClick={() => { setShowForm(true); setEditTarget(null); }} className="gap-1.5">
            <Plus className="h-4 w-4" />
            Thêm kho
          </Button>
        )}
          ...
          <div className="divide-y divide-border/40">
            {warehouses.map(w => (
              <div key={w.id} className="px-5 py-4">
                {editTarget?.id === w.id ? (
                  <WarehouseForm
                    initial={{ name: w.name, address: w.address, ward: w.ward, district: w.district, province: w.province, phoneNumber: w.phoneNumber }}
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {!w.isActive && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-muted text-muted-foreground uppercase">Tắt</span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button
                        onClick={() => { setEditTarget(w); setShowForm(false); }}
                        className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground transition-colors"
                      >
                        <Pencil className="h-4 w-4" />
                      </button>
                      <button
                        onClick={() => { if (confirm(`Xóa kho "${w.name}"?`)) deleteMutation.mutate(w.id); }}
