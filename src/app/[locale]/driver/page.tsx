...
export default function DriverPage() {
    ...
    for (const order of transitOrders) {
      ...
      if (!map.has(order.hub.id)) {
        map.set(order.hub.id, {
          ...
          hubFullAddress: [order.hub.address, order.hub.ward, order.hub.district, order.hub.city]
            .filter(Boolean)
            .join(', '),
          orderCount: 0,
          totalAmount: 0,
  ...
  const toggleHub = (hubId: string) => {
    setSelectedHubIds(prev => {
      ...
      if (base.has(hubId)) base.delete(hubId);
      else base.add(hubId);
      return base;
    });
    setRouteResult(null);
  ...
  const optimizeMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      if (errorCode === 'DRIVER_NO_WAREHOUSE') {
        toast.error(
          'Lỗi: Kho xuất phát của bạn đã bị vô hiệu hóa hoặc chưa được gán. Vui lòng liên hệ Admin!'
        );
      } else {
    ...
    <div className="max-w-3xl mx-auto py-6 px-4 space-y-5">
      ...
      {tab === 'pickup' &&
        (loadingPickup ? (
          ...
          <EmptyState
            icon={Package}
            title="Không có đơn cần lấy"
            sub="Các đơn đã thanh toán sẽ xuất hiện ở đây"
          />
        ) : (
          <div className="space-y-2">
      ...
      {tab === 'transit' &&
        (loadingTransit ? (
          ...
          <EmptyState
            ...
            sub="Đơn đã lấy từ kho đang vận chuyển đến Hub sẽ hiện ở đây"
          />
        ) : (
          <div className="space-y-2">
      ...
      {tab === 'history' &&
        (loadingHistory ? (
          ...
          <EmptyState
            ...
            sub="Lịch sử giao hàng trong ngày sẽ hiện ở đây"
          />
        ) : (
          <div className="space-y-2">
            <p className="text-xs text-gray-400 px-1">
              {historyOrders.length} đơn hoàn thành hôm nay
            </p>
        ...
        <div className="space-y-4">
          ...
          {loadingPickup || loadingTransit ? (
            ...
            <EmptyState
              ...
              sub="Cần có đơn đang giao hoặc chờ lấy để tính lộ trình"
            />
          ) : (
            <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
                ...
                <button
                  ...
                  className="text-xs text-purple-600 hover:underline font-medium"
                >
                  {effectiveSelectedHubIds.size === routeBatches.length
                    ? 'Bỏ chọn tất cả'
                    : 'Chọn tất cả'}
                </button>
              </div>
          ...
          {routeBatches.length > 0 && (
            <div className="bg-white border border-gray-100 rounded-xl p-4 space-y-3">
              ...
              {/* Trạng thái đang tải */}
              {loadingWarehouse && <Skeleton className="h-14 w-full rounded-lg" />}
              ...
              {!loadingWarehouse && isWarehouseError && (
                <div className="flex gap-3 p-3 rounded-lg border border-amber-200 bg-amber-50">
                  ...
                  <p className="text-sm text-amber-800 leading-snug">
                    Tài khoản của bạn chưa được gán cho kho cố định nào. Vui lòng liên hệ Admin để
                    được phân bổ trước khi tối ưu lộ trình.
                  </p>
                </div>
              )}
