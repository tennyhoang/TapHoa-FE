...
export default function AdminOrdersPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: () =>
      orderService.getAllOrders({ page, pageSize: PAGE_SIZE, status: statusFilter, search }),
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Table ── */}
      <div
        className={`bg-card rounded-2xl shadow-sm border border-border/60 overflow-hidden transition-opacity ${isFetching && !isLoading ? 'opacity-70' : ''}`}
      >
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border/60 bg-muted/40">
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider w-32">
                Mã đơn
              </th>
