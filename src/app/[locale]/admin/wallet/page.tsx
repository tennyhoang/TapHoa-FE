...
export default function AdminWalletPage() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get<WithdrawRequest[]>('/admin/wallet/withdraw-requests', { params: { status: tab } })
        .then(r => r.data),
  });
  ...
  const process = useMutation({
    mutationFn: ({ id, action }: { id: string; action: 'complete' | 'reject' }) =>
      api
        .patch(`/admin/wallet/withdraw-requests/${id}/${action}`, { note: note || undefined })
        .then(r => r.data),
    onSuccess: (_, { action }) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Xử lý yêu cầu rút tiền từ ví người dùng
        </p>
      </div>
          ...
          <div key={req.id} className="bg-card rounded-xl border border-border p-5 space-y-4">
            ...
            {req.processedAt && (
              <p className="text-xs text-muted-foreground">
                Xử lý lúc: {formatDate(req.processedAt)}
              </p>
            )}
          </div>
        ))}
