        ...
        <div className="divide-y divide-border/40">
          {items.map((item: AdminSessionItem) => (
            <div key={item.id} className="py-2.5 flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  if (confirm('Xóa sản phẩm này?')) removeItemMutation.mutate(item.id);
                }}
    ...
    <div className="px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            onClick={() => {
              if (confirm('Xóa phiên Flash Sale này và toàn bộ sản phẩm?')) deleteMutation.mutate();
            }}
...
export default function AdminFlashSalePage() {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      flashSaleService.admin.createSession({
        name,
    ...
    <div className="space-y-6 max-w-5xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Tạo phiên giảm giá có giới hạn thời gian và số lượng
        </p>
      </div>
