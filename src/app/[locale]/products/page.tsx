...
function CategoryCircle({
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-xs font-medium text-center leading-tight max-w-[56px] truncate transition-colors
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-1 mt-2 max-h-[60vh] overflow-y-auto pr-1">
          {categories.map(parent => {
            const parentActive =
              selectedId === parent.id || parent.children.some(c => c.id === selectedId);
            return (
              <div key={parent.id}>
                <button
                  type="button"
    ...
    <div className="space-y-0">
      ...
      <div className="mb-6">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 max-w-xl">
          ...
          <Button
            type="submit"
            className="bg-primary hover:bg-primary/90 text-primary-foreground h-10 px-5 rounded-xl font-semibold"
          >
            Tìm
          </Button>
        </form>
      ...
      {categories && categories.length > 0 && (
        <div className="mb-6">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
            ...
            >
              <div
                className={`w-14 h-14 rounded-2xl border-2 transition-all duration-150 flex items-center justify-center
                ${
                  !effectiveCategoryId
              ...
              >
                <span
                  className={`text-xs font-bold select-none ${!effectiveCategoryId ? 'text-primary-foreground' : 'text-muted-foreground'}`}
                >
                  Tất cả
                </span>
              </div>
              <span
                className={`text-xs font-medium transition-colors ${!effectiveCategoryId ? 'text-primary' : 'text-foreground/70'}`}
              >
                Tất cả
              </span>
            </button>
            {categories.map((cat, i) => (
              <CategoryCircle
                ...
                selected={
                  ...
                  cat.children.some(c => c.id === effectiveCategoryId)
                }
