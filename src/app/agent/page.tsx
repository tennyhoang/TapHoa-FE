...
function IncomingOrderCard({ order, onArrive, loading }: {
  order: Order; onArrive: () => void; loading: boolean;
}) {
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm font-medium text-gray-700 truncate">{order.items.length} sản phẩm · {formatPrice(order.totalAmount)}</p>
...
function ReadyOrderCard({ order, onComplete, loading }: {
  order: Order; onComplete: () => void; loading: boolean;
}) {
  return (
...
export default function AgentPage() {
  ...
  const TABS = [
    { key: 'incoming' as Tab, label: 'Đang về Hub',   count: incomingOrders.length, icon: Truck,          activeColor: 'text-purple-600', bg: 'bg-purple-100 text-purple-700' },
    { key: 'ready'    as Tab, label: 'Chờ khách lấy', count: readyOrders.length,    icon: Package,        activeColor: 'text-indigo-600', bg: 'bg-indigo-100 text-indigo-700' },
    { key: 'history'  as Tab, label: 'Lịch sử',       count: historyOrders.length,  icon: Clock,          activeColor: 'text-green-600',  bg: 'bg-green-100 text-green-700' },
  ];
    ...
    <div className="max-w-3xl mx-auto py-6 px-4 space-y-5">
      ...
      {tab === 'incoming' && (
        loadingIncoming ? (
          ...
          <EmptyState icon={Truck} title="Không có đơn nào đang về Hub" sub="Đơn được tài xế vận chuyển đến Hub sẽ xuất hiện ở đây" />
        ) : (
      ...
      {tab === 'ready' && (
        loadingReady ? (
          ...
          <EmptyState icon={Package} title="Không có đơn nào chờ giao" sub="Đơn đã về Hub và chờ khách đến lấy sẽ hiện ở đây" />
        ) : (
          <div className="space-y-2">
      ...
      {tab === 'history' && (
        loadingHistory ? (
          ...
          <EmptyState icon={Clock} title="Chưa có đơn hoàn thành" sub="Lịch sử giao nhận tại Hub sẽ hiện ở đây" />
        ) : (
          <div className="space-y-2">
